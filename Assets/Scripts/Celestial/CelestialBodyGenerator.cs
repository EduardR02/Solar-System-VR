...
public class CelestialBodyGenerator : MonoBehaviour {
	...
	bool registeredWithEnvironment;
	...
	void OnDisable () {
		DeregisterFromEnvironment ();
	}

	void Update () {
		if (Application.isPlaying && !registeredWithEnvironment) {
			TryRegisterWithEnvironment ();
		}
	...
	void InvalidateCachedData () {
		...
		TryRegisterWithEnvironment (); // ensure registry sees valid data once references exist
	}

	void TryRegisterWithEnvironment () {
		if (registeredWithEnvironment) {
			return;
		}
		if (!body || body.shape == null || body.shading == null) {
			return;
		}
		if (PlanetEnvironmentRegistry.Register (this)) {
			registeredWithEnvironment = true;
			PlanetEnvironmentRegistry.MarkDirty ();
		}
	}

	void DeregisterFromEnvironment () {
		if (!registeredWithEnvironment) {
			return;
		}
		PlanetEnvironmentRegistry.Unregister (this);
		registeredWithEnvironment = false;
		PlanetEnvironmentRegistry.MarkDirty ();
	}
